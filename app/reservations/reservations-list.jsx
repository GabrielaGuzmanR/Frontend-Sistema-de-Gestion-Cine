...
import { Calendar, Clock, MapPin, Ticket, User, Clapperboard } from "lucide-react"
    ...
    const fetchReservations = async () => {
      ...
      try {
        ...
        const formattedReservations = data.map((reservation) => ({
          ...
          date: reservation.Function.date.split("T")[0],
          time: reservation.Function.time.slice(0, 5),
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      {reservations.length === 0 ? (
        <div className="relative flex flex-col items-center justify-center py-16 bg-gradient-to-b from-gray-900/50 to-gray-800/50 rounded-2xl border border-gray-800/50 shadow-lg">
          {/* Fondo decorativo */}
          <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 rounded-2xl blur-3xl -z-10 opacity-50" />

          {/* Ícono grande */}
          <Clapperboard className="h-20 w-20 text-blue-400 mb-6 animate-pulse" />
          ...
          <p className="text-gray-300 text-center max-w-md mb-6">
            ¿Listo para disfrutar de una gran película? Explora nuestras funciones disponibles y reserva tus asientos para una experiencia inolvidable en el cine.
          </p>

          {/* Botón decorativo (sin redirección por ahora) */}
          <button className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-lg shadow-lg shadow-blue-700/30 transition-all duration-300 flex items-center gap-2">
            <Ticket className="h-5 w-5" />
            Ve a la sección de películas.
          </button>
        </div>
      ) : (
