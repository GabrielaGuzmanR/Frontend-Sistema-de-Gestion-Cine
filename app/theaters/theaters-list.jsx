  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (roomsResponse.status === 404) {
        // Si no hay salas, tratamos esto como un caso válido (lista vacía)
        roomsData = [];
      } else if (!roomsResponse.ok) {
        throw new Error(`Error al obtener salas: ${roomsResponse.status}`);
      } else {
      ...
      if (functionsResponse.status === 404) {
        // Si no hay funciones, tratamos esto como un caso válido (lista vacía)
        functionsData = [];
      } else if (!functionsResponse.ok) {
        throw new Error(`Error al obtener funciones: ${functionsResponse.status}`);
      } else {
      ...
      if (moviesResponse.status === 404) {
        // Si no hay películas, tratamos esto como un caso válido (lista vacía)
        moviesData = [];
      } else if (!moviesResponse.ok) {
        throw new Error(`Error al obtener películas: ${moviesResponse.status}`);
      } else {
    ...
    <div className="space-y-6 pb-6">
      ...
      {theaters.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 bg-white/5 rounded-2xl border border-dashed border-white/20">
          ...
          <p className="text-gray-300 text-lg mb-4">No hay salas disponibles</p>
          <Button
            onClick={() => setIsAddTheaterOpen(true)}
            className="bg-blue-600 hover:bg-blue-700 text-white"
          >
            <Plus className="mr-2 h-4 w-4" /> Añadir Sala
          </Button>
        </div>
      ) : (
